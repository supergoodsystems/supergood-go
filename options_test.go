...
func TestOptions_overrides(t *testing.T) {
	...
	o, err := (&Options{
		...
		RedactResponseBodyKeys:  map[string][]string{"example.com": []string{"responsebody.path.to.key"}},
		RedactRequestBodyKeys:   map[string][]string{"example.com": []string{"requestbody.path.to.key"}},
		RedactRequestHeaderKeys: map[string][]string{"example.com": []string{"header-key"}},
